...
class UpdateNotePinUseCase @Inject constructor(
	private val noteRepository: NoteRepository
) {
	operator fun invoke(noteId: Int, isPinned: Boolean): Flow<Either<NoteResponse>> = flow {
		val response = noteRepository.updateNotePin(noteId, isPinned)
		emit(response)
	}
}
